...
class Link {
  ...
  /**
   * Create signing link url for document
   * @typedef {Object} SigningLinkParams
  ...
  /**
   * Create signing link url
   * @param {SigningLinkParams} data - signing link payload
  ...
  /**
   * Create document signing link url for third-party app
   * @param {SigningLinkParams} data - signing link payload
   * @param { function(err: ApiErrorResponse, res: SigningLinkCreateResponse) } [callback] - error first node.js callback
   */
  static createUrl ({ document_id, token }, callback) {
    const JSONData = JSON.stringify({ document_id });
    const req = https
      .request(buildRequestOptions({
        method: 'POST',
