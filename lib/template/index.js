...
class Template {
   ...
   * @property {string} file - path to .csv file with roles and their emails
  ...
  static bulkInvite ({
    ...
    if (!isCSVFile(file)) {
      callback('The file that was tried to be upploaded has not CSV format');
      return;
    }
    ...
    let formDataBeginning;
    let fileContent;

    try {
      formDataBeginning = Object
        .entries(metaData)
        .reduce((formData, [
          name,
          value,
        ]) => `${formData}${formDataBoundary}Content-Disposition: form-data; name="${name}";\r\n\r\n${value}\r\n`)
        .concat(formData => `${formData}${formDataBoundary}`)
        .concat(formData => `${formData}Content-Disposition: form-data; name="file"; filename="${basename(file)}"\r\n`)
        .concat(formData => `${formData}Content-Type: text/csv\r\n\r\n`);
