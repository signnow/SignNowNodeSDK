...
class Document {
  ...
  /**
   * Signers list of document payload
  ...
  static signers ({ id, token }, callback) {
    promisify(Document.view)({
      ...
      .then(document => {
        ...
        const emptyArr = [];
        const fieldInvitesEmails = getSignersEmail(field_invites, 'email') || emptyArr;
        const routingDetailsEmails = getSignersEmail(routing_details.length && routing_details[0].data, 'default_email') || emptyArr;
        const signaturesEmails = getSignersEmail(signatures, 'email') || emptyArr;
        const waitingForEmails = getSignersEmail(waiting_for) || emptyArr;
        const requestEmails = getSignersEmail(requests, 'signer_email') || emptyArr;

        const signersEmails = new Set(
          [
            ...fieldInvitesEmails,
            ...routingDetailsEmails,
            ...signaturesEmails,
            ...waitingForEmails,
            ...requestEmails,
          ]);

        if (signersEmails.size <= 0) {
          console.error('No signers on specific document');
          return;
        }

        callback([...signersEmails]);
      })
