...
class Document {
  ...
  static create ({ filepath, token }, callback) {
    ...
    const boundaryKey = getRandomString(30);
    const mimeType = getMimeTypeByFileExtension(fileExt);
    const fileName = getFileNameFromPath(filepath);
    const fileReq = buildFileRequest(boundaryKey, mimeType, fileName);
    const fileSize = getFileSize(filepath);
    const closingLine = getRequestClosingLine(boundaryKey);
    ...
    const req = https
      .request(buildRequestOptions({
        ...
        headers: {
          ...
          'Content-Length': fileSize + fileReq.toString().length + closingLine.length,
        },
    ...
    const fileStream = fs.createReadStream(filepath, { bufferSize: 64 * 1024 });
    fileStream.on('end', () => {
