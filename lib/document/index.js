...
class Document {
   ...
   * @typedef {Object} DownloadDocumentOptions
   ...
   * @property {string} id - id of specific document
   * @property {string} token - your auth token
   * @property {DownloadDocumentOptions} options - download document optional settings
   */
   ...
   * By default document downloading without history. To download document with history set `withHistory` option to `true`.
  ...
  static download ({
    ...
    const path = withHistory
      ? `/document/${id}/download/collapsed?with_history=1`
      : `/document/${id}/download?type=collapsed`;
