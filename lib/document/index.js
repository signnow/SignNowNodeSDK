    ...
    promisify(Document.view)({
      ...
      .then(document => {
        ...
        if (Object.keys(options).length === 0) {
          const fieldInvitesEmails = getSignersEmail(field_invites, 'email') || [];
          const freeFormInvitesEmails = getSignersEmail(requests, 'signer_email') || [];
        ...
        if (freeFormInvites) {
          const freeFomInvites = getSignersEmail(requests, 'signer_email') || [];

          emails.push(...freeFomInvites);
        }

        if (fieldInviteStatus) {
          const fieldInvites = getSignersEmail(field_invites, 'email', fieldInviteStatus) || [];
        ...
        if (paymentRequestStatus) {
          const paymentRequests = getSignersEmail(field_invites, 'email', paymentRequestStatus) || [];
