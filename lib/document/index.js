...
class Document {
   ...
   */
  static update({id, fields, token}, callback) {
   ...
   * @property {boolean} [zip=false] - if false document will be downloaded with embedded field and element values else archived to zip
   * @property {boolean} [base64=false] - if true download document encoded in base64
   * @property {boolean} [withHistory=false] - if true document will be downloaded with history
   * @property {boolean} [noHistory=false] - if true don't save history and don't track download
   * @property {boolean} [withHistory=false] - if true document will be downloaded with history
   * @property {boolean} [skipWatermark=false] - if true skip document watermark
   */
  ...
  static download(
    ...
    zip ? (queryParams.type = 'zip') : (queryParams.type = 'collapsed');
    base64 ? (queryParams.base64 = true) : null;
    noHistory ? (queryParams.no_history = '1') : null;
    withHistory ? (queryParams.with_history = '1') : null;
    skipWatermark ? (queryParams.skip_watermark = true) : null;

    console.log(
      `/document/${id}/download?client_timestamp=${clientTimestamp}&${stringifyQueryParams(
        queryParams
      )}`
    );
    ...
    https
      .request(
        buildRequestOptions({
          method: 'GET',
          path: `/document/${id}/download?client_timestamp=${clientTimestamp}&${stringifyQueryParams(
            queryParams
          )}`,
          authorization: {
            type: 'Bearer',
   ...
   */
  static invite({id, data, options: {email} = {}, token}, callback) {
   ...
   */
  static merge(
    {name, document_ids, options: {removeOriginalDocuments = false} = {}, token},
    originalCallback
  ) {
    ...
    const callbackWithDocumentDeletion = (mergeErr, mergeRes) => {
      if (mergeErr) {
        ...
        const documentsDeleted = document_ids.map(id => removeDocument({id}));

        Promise.all(documentsDeleted)
          .then(deletionStatuses =>
            deletionStatuses.reduce(
              (generalStatus, currentStatus) => generalStatus && currentStatus,
              true
            )
          )
    ...
    const callback = removeOriginalDocuments ? callbackWithDocumentDeletion : originalCallback;
   ...
   */
  static remove({id: documentId, token, options: {cancelInvites = false} = {}}, originalCallback) {
          ...
          .then(({fieldInvites, freeformInvites}) => {
            const fieldInvitesCancelled = new Promise(resolve => {
              if (Array.isArray(fieldInvites) && fieldInvites.length) {
                resolve(
                  promisify(Document.cancelFieldInvite)({
                    id: documentId,
                    token
                  })
                );
            ...
            const freeformInvitesCancelled = freeformInvites.map(({id: inviteId}) => {
            ...
            Promise.all([fieldInvitesCancelled, ...freeformInvitesCancelled])
    ...
    const callback = cancelInvites ? callbackWithInviteCancellation : originalCallback;
