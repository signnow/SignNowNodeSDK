...
/**
 * Closing line for file multipart/form-data request
 * @param {string} boundaryKey
 * @return {string}
 */
const getRequestClosingLine = boundaryKey => `\r\n------FormBoundary${boundaryKey}--`;
 ...
 * @return {string}
 */
const getFileNameFromPath = filePath => filePath.lastIndexOf('/') !== -1
  ? filePath.substr(filePath.lastIndexOf('/')).replace('/', '')
  : filePath;
 ...
 * @return {string}
 */
const getFileExtensionFromPath = filePath => filePath.substr(filePath.lastIndexOf('.')).replace('.', '');
 ...
 * @return {string}
 */
// const getMimeTypeByFilePath = filePath => fileExtensionsToMimeTypesMap[getFileExtensionFromPath(filePath).toLowerCase()];

/**
 * Get file size by its path
 * @param {string} filePath
 * @return {string}
 */
 ...
 * @return {boolean}
 */
const isCSVFile = filePath => getFileExtensionFromPath(filePath).toLowerCase() === 'csv';
